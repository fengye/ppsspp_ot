...
void TextureCache::UpdateSamplingParams(TexCacheEntry &entry, bool force) {
	...
	bool noMip = (gstate.texlevel & 0xFFFFFF) == 0x000001 || (gstate.texlevel & 0xFFFFFF) == 0x100001 || (gstate.texlevel & 0xFFFFFF) == 0x200001 || (gstate.texlevel & 0xFFFFFF) == 0x400001;  // Fix texlevel at 0
...
void TextureCache::SetTexture(bool force) {
	...
	// Detected Texture Level
	u8 level = ((gstate.texlevel >> 16) & 0xFF) / 16;
	...
	LoadTextureLevel(*entry, level, replaceImages, scaleFactor, dstFmt);
...
void TextureCache::LoadTextureLevel(TexCacheEntry &entry, int level, bool replaceImages, int scaleFactor, GLenum dstFmt) {
	...
	if (replaceImages) {
		...
		//forced 0 to correct characters show
